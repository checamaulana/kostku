...
export const createKost = async (
  kostData: KostFormData
): Promise<Kost | null> => {
  try {
...
export const searchKost = async (
  searchTerm: string,
  typeFilter?: "putra" | "putri" | "campur"
): Promise<Kost[]> => {
  try {
    let query = supabase
      .from("kost")
